  ...
  try {
    ...
    if (!companyName || !email || !taxId) {
      return res.status(400).json({
        error: 'Faltam dados obrigatorios para iniciar a assinatura.'
      });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      return_url: `${baseUrl}/?tab=assinatura&checkout=success&session_id={CHECKOUT_SESSION_ID}`,
    ...
    console.error('[Stripe] Erro ao criar checkout embutido de assinatura:', error);
    ...
    return res.status(500).json({
      error: 'Falha ao iniciar o checkout embutido da Stripe.',
