...
import {
  HotelFormEmailPayload,
  HotelFormImagePayload,
  HotelFormPhonePayload,
  HotelFormSubmission
} from './hotel-form.types';
...
export class HotelFormComponent implements OnInit {
    ...
    @Output() save = new EventEmitter<HotelFormSubmission>();
    ...
    ngOnInit() {
        ...
        if (this.hotelToEdit) {
            ...
            this.photosLink.set(this.hotelToEdit.photos_link || '');
    ...
    onSubmit() {
        ...
        const baseData = {
            ...
            photos_link: this.photosLink() || undefined
        };

        // New items do not have an ID yet
        const persistedEmails: HotelFormEmailPayload[] = finalEmails
            .filter(e => !!e.email && !!e.type)
            .map(e => ({ id: e.id, email: e.email!, type: e.type! }));
        const persistedPhones: HotelFormPhonePayload[] = finalPhones
            .filter(p => !!p.phone)
            .map(p => ({ id: p.id, phone: p.phone!, is_whatsapp: !!p.is_whatsapp }));
        const persistedImages: HotelFormImagePayload[] = finalImages
            .filter(i => !!i.image_url)
            .map(i => ({ id: i.id, image_url: i.image_url!, description: i.description }));

        const newEmails: HotelFormEmailPayload[] = persistedEmails
            .filter(e => !e.id)
            .map(e => ({ email: e.email, type: e.type }));
        const newPhones: HotelFormPhonePayload[] = persistedPhones
            .filter(p => !p.id)
            .map(p => ({ phone: p.phone, is_whatsapp: p.is_whatsapp }));
        const newImages: HotelFormImagePayload[] = persistedImages
            .filter(i => !i.id)
            .map(i => ({ image_url: i.image_url, description: i.description }));

        this.save.emit({
            hotelData: baseData,
            emails: this.hotelToEdit ? persistedEmails : newEmails,
            phones: this.hotelToEdit ? persistedPhones : newPhones,
            images: this.hotelToEdit ? persistedImages : newImages,
            newImages: newImages,
